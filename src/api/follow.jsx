...
export const unfollowUser = async (userId) => {
  ...
  try {
    const response = await axios.delete(
      `${baseUrl}/api/v1/unfollow/${userId}`,
      config
    );
...
export const getMyFollowers = async (size = 10, page = 1) => {
  ...
  try {
    const response = await axios.get(
      `${baseUrl}/api/v1/my-followers?size=${size}&page=${page}`,
      config
    );
...
export const getFollowingByUserId = async (userId, size = 1000, page = 1) => {
  ...
  try {
    ...
    console.error(
      "Error fetching following by user ID:",
      error.response?.data || error.message
    );
    throw error;
  }
...
export const getFollowersByUserId = async (userId, size = 1000, page = 1) => {
  ...
  try {
    ...
    console.error(
      "Error fetching followers by user ID:",
      error.response?.data || error.message
    );
    throw error;
  }
