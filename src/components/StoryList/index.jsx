...
const StoryList = () => {
  ...
  return (
    <div className="max-w-xs p-3 md:max-w-2xl">
      {stories.length === 0 ? (
        <p className="px-3 py-16 ml-3 text-sm text-center text-gray-500">
          No stories found.
        </p>
      ) : (
        <main className="">
          <div className="flex w-full space-x-3 overflow-x-auto">
            {stories.map((story, index) => (
              <div
                key={index}
                className="flex-shrink-0 w-20 text-center snap-start"
              >
                <div
                  className="relative cursor-pointer"
                  onClick={() => openModal(story)}
                >
                  <div className="p-0.5 rounded-full bg-gradient-to-tr from-purple-500 to-pink-500">
                    <img
                      src={
                        story.user?.profilePictureUrl ?? "/fallback-avatar.png"
                      }
                ...
                <p className="pt-1 text-sm text-black truncate">
