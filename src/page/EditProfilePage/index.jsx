...
const EditProfilePage = () => {
  ...
  const {
    ...
    loading: uploading,
    error: uploadError,
  } = useUploadImage(); // Get the hook's data
  const {
    updateProfile,
    loading: updating,
    error,
  ...
  const handleUpdate = async (e) => {
    ...
    const updatedForm = {
      ...form,
      profilePictureUrl: imageUrl || form.profilePictureUrl,
    };
    ...
    <main>
      ...
      <div className="flex items-center justify-center min-h-screen px-4 py-20 bg-gray sm:px-6 lg:px-8">
        <div className="w-full max-w-md p-8 bg-white rounded-lg shadow-lg opacity-80">
          <BackButton />
          <h1 className="mb-6 text-xl font-bold text-center text-black">
            Edit Profile
          </h1>
          ...
          <div className="flex flex-col items-center mb-6">
            <label
              ...
              className="inline-block px-4 py-2 text-white bg-blue-500 rounded-md cursor-pointer hover:bg-blue-700"
            >
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            {["name", "username", "email", "phoneNumber", "website"].map(
              (field) => (
                <div key={field}>
                  <label
                    htmlFor={field}
